...
async function assertProjectConfigDirectory(directory: string): Promise<void> {
  try {
    const metadata = await lstat(directory);
    if (metadata.isSymbolicLink()) {
      throw new Error("Symlinked workspace .omp directories cannot be edited through Paseo.");
    }
    if (!metadata.isDirectory()) throw new Error("Workspace .omp path is not a directory.");
  } catch (error) {
...
async function writeProjectChanges(
  ...
  await assertProjectConfigDirectory(join(cwd, ".omp"));
...
async function markWorkspaceOverrides(cwd: string, settings: OmpSetting[]): Promise<OmpSetting[]> {
  ...
  let config: unknown;
  try {
    legacy = JSON.parse(await readFile(join(directory, "settings.json"), "utf8")) as unknown;
  } catch {
    // Legacy project settings are optional.
  }
  try {
    config = parseYaml(await readFile(join(directory, "config.yml"), "utf8")) as unknown;
  } catch {
    // Canonical project settings are optional.
  }
  return settings.map((setting) => {
    const inLegacy = hasConfiguredPath(legacy, setting.path);
    const inConfig = hasConfiguredPath(config, setting.path);
    if (!inLegacy && !inConfig) return setting;
    return {
      ...setting,
      workspaceOverride: true,
      workspaceOverrideSource: inLegacy && inConfig ? "both" : inLegacy ? "legacy" : "config",
    };
  });
}
  ...
  if (input.cwd) {
    for (const change of input.changes) {
      ...
      if (
        ...
        (setting.workspaceOverrideSource === "legacy" || setting.workspaceOverrideSource === "both")
      ) {
        return {
          conflict: false,
          appliedPaths: [],
          failed: {
            path: change.path,
            message:
              "This override also exists in .omp/settings.json. Remove the legacy value directly before resetting it in Paseo.",
          },
          catalog: current,
        };
      }
