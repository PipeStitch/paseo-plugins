...
describe("OMP scalar settings updates", () => {
  ...
  test("creates and removes validated workspace overrides without global mutations", async () => {
    ...
    try {
      ...
      expect(
        listed.settings.find((setting) => setting.path === "personality")?.workspaceOverrideSource,
      ).toBe("config");
  ...
  test("does not claim legacy overrides were removed", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "paseo-omp-settings-"));
    await mkdir(join(cwd, ".omp"), { recursive: true });
    const legacyPath = join(cwd, ".omp", "settings.json");
    await writeFile(legacyPath, '{"personality":"legacy"}\n');
    const { dependencies } = harness();
    try {
      const listed = await listOmpSettingsWithDependencies({ cwd }, dependencies);
      const setting = listed.settings.find((candidate) => candidate.path === "personality");
      expect(setting?.workspaceOverrideSource).toBe("legacy");
      const result = await updateOmpSettingsWithDependencies(
        {
          cwd,
          revision: listed.revision ?? "",
      ...
      expect(result.failed?.message).toContain(".omp/settings.json");
      expect(await readFile(legacyPath, "utf8")).toBe('{"personality":"legacy"}\n');
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  test.skipIf(process.platform === "win32")(
    "rejects a symlinked workspace config directory",
    async () => {
      const cwd = await mkdtemp(join(tmpdir(), "paseo-omp-settings-"));
      const target = await mkdtemp(join(tmpdir(), "paseo-omp-shared-settings-"));
      await writeFile(join(target, "config.yml"), "personality: concise\n");
      await symlink(target, join(cwd, ".omp"), "dir");
      const { dependencies } = harness();
      try {
        const listed = await listOmpSettingsWithDependencies({ cwd }, dependencies);
        const result = await updateOmpSettingsWithDependencies(
        ...
        expect(result.failed?.message).toContain("Symlinked workspace .omp directories");
        expect(await readFile(join(target, "config.yml"), "utf8")).toBe("personality: concise\n");
      } finally {
        await Promise.all([
